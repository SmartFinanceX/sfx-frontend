    ...
    <v-container>
      <v-card flat>
        <v-card-title class="d-flex align-center pe-2" style="font-weight: 900">
          ...
          <v-spacer></v-spacer>
          <v-text-field v-model="search" prepend-inner-icon="mdi-magnify" density="compact" label="查找指定股票" single-line
        ...
        <v-divider></v-divider>
        <v-data-table :headers="headers" :items="this.tradeHistory" :search="search" :items-per-page="10"
          loading-text="数据加载中..." no-data-text="你还暂时没有记录交易..." items-per-page-text="每页显示数量" density="comfortable">
...
export default {
  ...
  data() {
    return {
      ...
      search: "",
      headers: [
        { align: "start", key: "ticker", sortable: false, title: "股票代码" },
        { key: "price", title: "成交价格" },
        { key: "tp", title: "交易类型" },
        { key: "time", title: "交易时间" },
        { key: "num", title: "交易单位量" },
        { key: "fee", title: "总手续费" },
  ...
  methods: {
    ...
    getTradeHisroty() {
      var requestUrl = '/api/analyze/benefit/query_by_user/' + this.userId;
