    ...
    <v-container>
        ...
        <v-container v-show="searched">
            ...
            <v-bottom-navigation class="align-center">
                ...
                <v-chip variant="text" v-on:click="addpage" v-show="page < maxpage" prepend-icon="mdi-skip-next"
                    rounded="xl">
                    下一页
                </v-chip>
...
export default {
    ...
    methods: {
        ...
        search(way, msg2) {
            ...
            // console.log("search")
            if (way == "关键词") {
                const url = `http://124.222.191.199:9000/_search/_all/${msg2}/${this.page}`;
                console.log(url);
                let that = this
            ...
            if (way == "股票代码") {
                const url = `http://124.222.191.199:9000/_search/_ticker/${msg2}`;
                console.log(url);
                let that = this
                this.$http.get(url).then((res) => {
                    if (res.data.code != 200) {
